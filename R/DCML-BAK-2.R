...
SMPY <- function(mf, y, control, split, corr.b=control$corr.b) { 
  ...
  res <- tmp$residuals
  sih <- sspy
  # r2 <- as.vector(y - cbind(X, Z) %*% as.vector(c(betapy, gammapy2)) )
  # sih2 <- mscale(u=r2, tol=1e-7, delta=dee, tuning.chi=control$tuning.chi)
  # print(beta)
  # sih <- sspy
