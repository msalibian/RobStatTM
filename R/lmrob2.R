           ...
           control = lmrob2.control(), candidates='PY')
  {
    ...
    if (is.empty.model(mt)) {
      ...
      if (rankQR > 0) {
        ...
        # print(names(z))
        # [1] "coefficients"   "scale"          "residuals"      "loss"           "converged"      "iter"          
        # [7] "fitted.values"  "rweights"       "control"        "init"           "qr"             "rank"          
        # [13] "cov"            "df.residual"    "degree.freedom"
        # > 
...
summary.lmrob2 <- function(object, correlation = FALSE, symbolic.cor = FALSE, ...)
{
  ...
  #     ans$control$eps.x(max(abs(o.x))) ## else NULL
  structure(ans,
            class = "summary.lmrob2")
}



print.summary.lmrob2 <-
  function (x, digits = max(3, getOption("digits") - 3),
            symbolic.cor = x$symbolic.cor,
            signif.stars = getOption("show.signif.stars"), ...)
  {
