...
\item \textbf{xts}
\end{itemize}
With the above packages all installed, just use the command \texttt{ShinyUI()} at the prompt, to start the Shiny UI application.
Then you will be ready to start using the Shiny UI.
...
Figure \ref{fig:home} displays the initial screen you encounter in the Shiny UI application. This page is where you would select or upload a data set,
and then navigate to one of the model tabs for further analysis. The subsequent sections contain detailed instructions for each component of the Shiny UI.
...
and the data will be displayed in a table on the right. Figure 2 shows the
result for the data set mineral in RobStatTM.
...
From that location, the file is located in \textit{etc} subdirectory. For datasets with a large number
...
the side panel on the left, when default values  \textbf{Psi
...
scale for the variable called V1 in the data set \textbf{flour} (which you need to load) using for for the robust estimate the bisquare score function and an asymptotic
efficiency of 90\%.
...
In the example, zinc is the dependent variable and copper is the independent variable
for the LS and MM and for the MM estimator we are using the optimal value
...
fm <- fit.models(LS = lm(zinc ~ copper, data = mineral),
                 MM = lmrobdetMM(zinc ~ copper, data = mineral, 
                                 control = lmrobdet.control(family = "optimal",
                                                            eff    = 0.99)))
...
QQ Plot}, \emph{Estimated Residual Density}, \emph{Std. Residuals v. Robust Distances},
...
\caption{LS and robust MM normal QQ plots of residuals: \textbf{mineral}}
\label{f8}
\end{figure}
...
The above, along with figures \ref{f14}, \ref{f15}, and \ref{f16}, follow Example 6.2 and Figure 6.3 in Maronna et al. You
...
cov.fm <- fit.models(Classic = covClassic(dat),
...
Matrix}, \emph{Distances Chi-Squared QQ Plots}. Below are examples
of each plot:
...
\begin{figure}[H]
\centering
\includegraphics[width=6.5in]{img/CovarianceMd.png}\caption{Square-root Mahalanobis distances for classical and robust covariance estimates:
\textbf{wine}}
\label{f14}
\end{figure}
...
\caption{Classical and robust square-root Mahalanobis distances chi-squared QQ plots:
\textbf{wine}}
\label{f15}
\end{figure}
...
\caption{Scatterplot of square-root classical and robust Mahalanobis distances: \textbf{wine}}
\label{f16}
\end{figure}

The following figures are from analysis using selected variables in the \textbf{wine} dataset.
...
\caption{Scree plot for classical and robust covariance estimates: \textbf{wine}}
\label{f17}
\end{figure}
...
\textbf{wine}. If you repeat the analysis with just three (or four) well-chosen variables,
...
pca.fm <- fit.models(Classic = prcomp(bus[, 1:10]),
                     Robust  = prcompRob(bus[, 1:10]))
...
The Shiny UI will continue to evolve as we receive more feedback on
...
\subsection*{Additional Modeling Capability}

Future versions will also contain additional robust modeling methods. Currently,
we plan to add interfaces to robust logistic regression and robust
time series analysis as discussed in Chapters 7 and 8 of the book.
