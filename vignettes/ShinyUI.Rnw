...
Figure \ref{fig:home} displays the initial screen you encounte in the Shiny UI application. This page is where you would select or upload a data set,
...
fm <- fit.models(LS = lm(zinc ~ copper, data = mineral),
                 MM = lmrobdetMM(zinc ~ copper, data = mineral, 
                                 control = lmrobdet.control(family = "modopt",
                                                            eff    = 0.99)))
...
of classical and robust covariance estimates for the some variables in the \textbf{wine}
data set.
...
\textbf{hfunds.ts}. If you repeat the analysis with just three (or four) well-chosen variables,
...
\begin{figure}[H]
  \centering
    \includegraphics[width=6.5in]{img/PcaSummary.png}
