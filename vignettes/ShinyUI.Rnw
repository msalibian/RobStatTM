...
\item \textbf{xts}
\end{itemize}
With the above packages all installed, just use the command ShinyUI() at the prompt, to start the Shiny UI application.
...
scale for the \textbf{flour} dataset from 
where we used the bisquare score function and an asymptotic
efficiency of 90\%. In a later version, you will be able to select
...
\begin{figure}[H]
\centering
\includegraphics[width=6.5in]{img/CovarianceMd.png}\caption{Mahalanobis distances for classical and robust covariance estimates:
\textbf{wine}}
\label{f14}
\end{figure}
...
\caption{Classical and robust Mahalanobis distances chi-squared QQ plots:
\textbf{wine}}
\label{f15}
\end{figure}
...
\caption{Scatterplot of classical and robust Mahalanobis distances: \textbf{wine}}
\label{f16}
\end{figure}
